...
# Read layer for OK water service boundaries, clean, transform CRS
ok_wsb <- st_read(path(data_path, "boundary/ok/ok.geojson")) %>% 
  # clean whitespace
...
cat("Read OK boundary layer; cleaned whitespace; corrected geometries.\n ")
...
dir_create(path(staging_path, "ok"))

# delete layer if it exists, then write to geojson
path_out <- path(staging_path, "ok/ok_wsb_labeled.geojson")
