...
We call PWSIDs that report coordinates in a particular state but that actually plot in a different state "impostors" because they are not where they say they are. These errors likey result from faulty geocoding. Imposters pose a challenge to broader project aims because they will "decenter" predicted boundaries, spatially placing them in the wrong state. We assess imposters in ECHO, FRS, and mobile home parks (MHP).

```{r}
# a more robust version of this function is in src/functions/f_drop_imposters.R
# and is used in transformers to clean data
f_get_imposters <- function(x){
  # pull usa state geometries, project to input data CRS
  usa = USAboundaries::us_states(resolution = "high") %>% 
    st_transform(st_crs(frs)$epsg) %>% 
    select(state_intersection = state_abbr, geometry) %>% 
    suppressMessages()
    
  # join frs to us boundaries and return imposter geometries
  imposters <- st_join(x, usa) %>% 
...
# light cleaning that needs to move to transformers
frs <- frs %>% 
  mutate(
