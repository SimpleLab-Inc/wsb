...
# calculate overlap
f_intersects <- function(x, y){
  res <- st_intersects(
    filter(wsb_labeled, pwsid %in% matches$pwsid[x]), 
    filter(tiger, geoid %in% matches$tigris_match_geoid[y])
  )
  return(res)
}

st_intersects(filter(wsb_labeled, pwsid %in% matches$pwsid[5]), 
                filter(tiger, geoid %in% matches$tigris_match_geoid[5])) %>% unlist() %>% class()

st_intersects(filter(wsb_labeled, pwsid %in% matches$pwsid[c(3,5)]), 
                filter(tiger, geoid %in% matches$tigris_match_geoid[c(3,5)]))
```

```{r}
mapview(filter(wsb_labeled, pwsid == matches$pwsid[5]), 
        layer.name = "actual", col.regions = "red") + 
  mapview(filter(tiger, geoid == matches$tigris_match_geoid[5]), 
          layer.name = "TIGER")
```
