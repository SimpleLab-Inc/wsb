...
Centroid accuracy is more important for _unlabeled systems that we do not have explicit boundary polygons for, or that we cannot assign to a TIGRIS proxy polygon_ because it is about this centroid that a water service boundary radius will be estimated. Currently, we match TIGRIS place polygons to ECHO CWSs as a proxy for missing water system boundaries and achieve a `r round((nrow(filter(jc, !is.na(tigris_match_geoid)))/nrow(jc))*100, 2)`% match.

Thus, we now recalculate missingness for the subset of data **without** a matching TIGRIS place using the same two plots and table in the previous section.
...
In the plot below, note that blank states are those for which we have labeled data. States with results only show the PWSIDs without a matching TIGRIS place proxy polygon. 
