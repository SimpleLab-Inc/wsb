...
# Bring in all data for population counts and summaries
# j stands for joined data, read and rm rownumber column
j <- read_csv(path(staging_path, "matched_output.csv"), col_select = -1)

```

Labeled spatial data (i.e., spatial polygons present) are present for `r nrow(wsb_labeled_simp)` PWSIDs that cover `r length(unique(wsb_labeled_simp$state))` states: `r paste(unique(wsb_labeled_simp$state), collapse = ", ")`^[Due to constraints on visualizing the large number of vertices in these polygon data, the map shown below provides a low-resolution version of the labeled data, with only ~5% of vertices, but analyses all use the high-resolution version.]. This represents `r round(nrow(wsb_labeled_simp)/nrow(j)*100,2)`% of active, community water systems, serving `r j %>% filter(pwsid %in% wsb_labeled_simp$pwsid) %>% summarize(pop = sum(population_served_count)) %>% pull(pop)` people, or `r round((j %>% filter(pwsid %in% wsb_labeled_simp$pwsid) %>% summarize(pop = sum(population_served_count)) %>% pull(pop))/(j %>% summarize(full_pop = sum(population_served_count)) %>% pull(full_pop))*100,2)`% of people served by active, community water systems.
...
`r round((nrow(filter(j, service_connections_count == 0))/nrow(j))*100, 2)`% of PWSIDs have 0 service connections, which is unlikely to be true and primarily impacts SC. We may drop these, or impute service connections equal to the mean connections for the reported system size if present. 
...
The following table shows the counts of systems with a reported population of zero, by state.
```{r}
zero_pop %>% 
  count(primacy_agency_code, sort = TRUE, name = "n_missing") %>% 
  dt_make()
...
Take for example, 2 sets impostors in FRS data below. Each map presents two centroids of facilities that are supposed to belong to the same water system. The first map is for a system in California, which has an "imposter" centroid in Indiana. The second map is for a system in Alaska, which has an "imposter" centroid in Washington state.
...
# TIGER proxy polygon appropriateness
...
f_intersects <- function(x){
  ...
  # intersect labeled geometries with tigris polygon matches 
  # (based of course, on the current input: combined_data.csv matching algorithm)
  res = st_intersects(
  ...
  # a non-intersection returns `integer(0)` - a zero length vector - so we
  # replace with a true NA
  if(length(res) == 0)
    res = NA 
  
  # for systems with multi polygon, there is return vector > length 1 - so we
  # make the intersection TRUE (1) if at least one polygon intersects
  if(length(res) > 1)
    res = ifelse(1 %in% res, 1, NA)

  return(res)
}

# intersection across all labeled polygons - takes a while
...
```{r}
# non intersections (ni)
ni <- matches[which(is.na(lti)), ]
```

Let's now examine some of the non-intersections to understand their generating mechanism. There are `r nrow(ni)` non-intersections between TIGER places and labeled boundaries (recall, we aim to _validate_ the matching process here). Most non-intersections are spatially close enough where adding a small buffer would ensure an intersection. For all intents and purposes, these polygons match. For example see below:

```{r}
# non intersections (ni)
ni <- matches[which(is.na(lti)), ]

mapview(filter(wsb_labeled, pwsid == ni$pwsid[1]), 
        layer.name = "actual", col.regions = "red") + 
  mapview(filter(tiger, geoid == ni$tigris_match_geoid[1]), 
          layer.name = "TIGER")

mapview(filter(wsb_labeled, pwsid == ni$pwsid[30]), 
        layer.name = "actual", col.regions = "red") + 
  mapview(filter(tiger, geoid == ni$tigris_match_geoid[30]), 
          layer.name = "TIGER")
```
...
ECHO coverage is ubiquitous, which implies that predicted service boundaries about ECHO centroids will attain similar population served coverage as ECHO (~300 million people). However, we have seen above that [ECHO centroid accuracy](#centroid-coverage) is less accurate in regions with a county or state centroid level accuracy, which will lead to a "pancake problem" -- essentially, predicted circular boundaries will overlap at the county or state centroid location, like a stack of pancakes. Thus, improving centroid data quality is imperative. 
