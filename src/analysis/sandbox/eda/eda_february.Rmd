...
First, observe 2 examples of impostors in FRS data. Impostors need to be dropped and audited here because taking centroids of centoids using FRS data will be skewed by data 
...
Similar impostors exist in FRS and MHP datasets and have already been addressed in the data pipeline. Overall, imposter count and proprotion were small, and most notable in FRS data:

- FRS: 68,536 / 69,722 rows are valid ( 98.3 % of input data). 1,186 imposters.  
- ECHO: 426,195 / 426,545 rows are valid ( 99.9 % of input data). 350 imposters.  
- MHP: 45,636 / 45,642 rows are valid ( 99.99 % of input data). 6 imposters.  


> **RECOMMENDATION**: Because they are so few in number, we advise drop imposters to prevent "decentering" error caused by incorrect impostor centroids, and will review/audit output log files to clean additional imposters should they be relevant to the prediction task at hand.  
