...
temm <- dir_ls(here("temm_layer"), glob = "*geojson") %>% 
...
[^3]: Some water systems were excluded from this data layer. Specifically, 354 water systems were dropped because they had reported service connections less than 15 or population count less than 25, which we categorize as unreasonable given that community water systems are defined as serving at least 15 connections and 25 population served. Next, we removed 834 water systems associated with US territories; most excluded water systems in this step are found in Puerto Rico.
...
Together, around `r str_sub(sum((as.numeric(str_remove_all(pop$count[1:3], ",")))), 1, 3)` million people (`r sum(pop$prop[1:3])`% of the population) are covered by either a Tier 1 or a Tier 2 spatial boundary. The remaining approximately `r str_split(pop$count[4], ",", simplify = TRUE)[1]` million people (`r pop$prop[4]`%) are covered by a Tier 3 boundary. These results indicate high confidence in the spatial accuracy of the resulting TEMM water boundary layer for community water systems.
