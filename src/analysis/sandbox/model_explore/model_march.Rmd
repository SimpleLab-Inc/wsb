...
In total, the TAMM data layer represents tap water delivery to `r round(pop_total/1e6, 2)` million people served by `r format_bign(nrow(d))` water systems[^2].
...
The national water service boundary layer is too large to plot in this interactive report, thus we show a static map below to illustrate the coverage provided by the Tiers 1-3 in the proportions described above, and direct the reader to the TAMM spatial layer in the project repository.  

```{r tamm nationwide wsb}
include_graphics(here("src/analysis/sandbox/model_explore/etc/tamm-nation.png"))
```
...
```{r tamm zoom ca nv or}
# read TAMM output
tamm <- here("tamm_layer/2022-03-26_tamm.geojson") %>% 
  st_read(quiet = TRUE) %>% 
...
We assume that data provided by states is correct, and do not clean Tier 1 data beyond _assimilating_ the data across states into a single layer. 
...
The median distance between matching Tier 1 and 2 geometries is `r round(median(zd), 2)` and the interquartile range is `r round(quantile(zd, 0.25), 2)` - `r round(quantile(zd, 0.75), 2)`. This suggests that non-spatially intersecting matches are _still very nearby_ and hence, may be considered adequate preliminary water service boundaries. However, if spatial accuracy is paramount, it may be determined that Tier 3 boundaries should be used instead of Tier 2 boundaries, specifically in cases where Tier 2 boundaries do not spatially intersect the provided ECHO centroid. 
