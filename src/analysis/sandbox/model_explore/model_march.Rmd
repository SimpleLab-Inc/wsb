...
In total, the TAMM data layer represents tap water delivery to `r round(pop_total/1e6, 2)` million people served by the `r nrow(d)` water systems. 
...
`r str_sub(pop$count, 1, 3)[1]` million people are served by a Tier 1 explicit data layer, which is impressive for only `r wsb_labeled_clean$state %>% unique() %>% length()` states that provided explicit boundary data. However, these states (`r wsb_labeled_clean$state %>% unique() %>% sort() %>% paste(collapse = ", ")`) include notable high-population states like CA, TX, and PA. 

Together, `r str_sub(sum((as.numeric(str_remove_all(pop$count[1:2], ",")))), 1, 3)` million people (`r sum(pop$prop[1:2])`% of the population) are served by either a Tier 1 or a Tier 2 spatial boundary. The remaining ~`r str_split(pop$count[3], ",", simplify = TRUE)[1]` million people (`r pop$prop[3]`%) are served by a Tier 3 system. These results indicate high confidence in the spatial accuracy of the resulting TAMM water boundary layer for community water systems. 
...
Below, we show the Proportion of population coverage per TAMM Tier on a state-by-state basis. Notably:

- When Tier 1 data (orange bars below) is present, it tends to cover the vast majority of population in the state.  
- Tier 2 covers more population than Tier 3 across all states.  
- Tier 3 coverage is not uniform across states. This implies that the Tier 2 matching algorithm may perform better in some states, and may depend on factors that vary across states, like centroid accuracy and water system type (e.g., municipal). 
...
dg <- d %>% 
  ...
  # calculate the proportion of population served per state and tier
  summarise(prop = sum(population_served_count)/state_pop) %>% 
  ungroup() %>% 
  distinct() %>% 
