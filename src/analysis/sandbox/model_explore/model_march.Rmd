...
-   A nationwide water service boundary layer is provided for `r nrow(d)` community water systems covering a total population of `r format_bign(pop_total)`.\
...
-   `r round(100*(nrow(filter(temm, !is.na(tiger_match_geoid))) / nrow(temm)), 2)`% of water systems match a TIGER place by spatial intersection or name match.\
...
zjd <- map_dbl(1:nrow(t1_zero), ~f_distance(.x))
...
Population served and service connections are highly correlated ($r$ $=$ `r round(tidy(cor.test(d$population_served_count, d$service_connections_count))$estimate, 2)`) and exhibit log-normality. Thus, like the response variable, they are log-transformed, which further impacts the interpretation of the regression fit[^6].
