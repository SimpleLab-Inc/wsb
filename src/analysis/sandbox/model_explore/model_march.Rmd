...
Here, we zoom into California, Nevada, and Oregon -- four states with high proportions of Tier 1, 2, and 3 spatial boundaries respectively. The Tier 3 circular buffers in this interactive map represent median (50th percentile) model estimates. Click on the layers icon to toggle between Tiers.
...
Where water systems match to the same TIGER boundary, the proxy boundary for those systems is perfectly overlapping. This applies to `r temm %>% filter(tier == "Tier 2") %>% summarise(n = n_distinct(pwsid)) %>% pull(n)`
