...
```{r conceptual model, echo = FALSE, fig.cap = "Fig 1. Conceptual diagram of water service boundary tiers"}
...
Together, around `r sum(sys$count[1:2])` community water systems (`r sum(sys$prop[1:2])`% of systems in layer) are covered by either a Tier 1 or a Tier 2a spatial boundary. The remaining approximately `r sum(sys$count[3:4])` water systems (`r sum(sys$prop[3:4])`%) are covered by a Tier 2b or Tier 3 boundary.
...
```{r geofacet temm tier distribution per state, fig.cap = "Fig 2. TEMM Tier distribution by state"}
...
```{r facet temm distribution native american, fig.cap = "Fig 3. TEMM Tier distribution of systems in Native American territories-by EPA agency"}
dg %>% 
...
```{r datatable temm tier distribution per state, fig.cap="Table 4. TEMM tier distribution by state"}
dg %>% 
  mutate(
...
```{r temm statewide system size, fig.cap = "Fig 4. TEMM Tier distribution by system size"}
...
```{r temm statewide system size native american, fig.cap = "Fig 5. TEMM Tier distribution of systems in Native American territories-by system size"}
dg2 %>% 
  filter(! primacy_agency_code %in% c(state.abb, "DC")) %>% 
  ggplot(aes(size_class, prop, fill = tier)) + 
  geom_col(position = "stack") + 
...
```{r datatable temm tier distribution by system size, fig.cap = "Table 6. TEMM Tier distribution by system size" }
# dataframe for barplot geofacet: prop of each each tier per size class
d %>% 
  mutate(
    size_class = case_when(
...
```{r datatable temm tier distribution per state and system size, fig.cap = "Table 7. TEMM Tier distribution by system size and primacy agency code"}
dg2 %>% 
  mutate(
    prop  = ifelse(is.na(prop), 0, prop),
    prop  = round(prop, 3)
...
```{r temm nationwide wsb, fig.cap = "Fig 6. Nationwide TEMM water system boundary layer "}
...
```{r tier 1 presence absence, fig.cap = "Fig 7. Presence/absence of explicit boundaries in US"}
...
```{r jacard histogram, fig.cap = "Fig 8. Histogram of modified jacard similarity index"}
t1 %>% 
  filter(jacard != 0) %>% 
  ggplot(aes(jacard)) +
...
```{r jacard zeros how far from one another, fig.cap = "Fig 9. Distance between matched Tier 1 and Tier 2 polygon centroids"}
...
```{r radius tier 1 distribution, fig.cap = "Fig 10. Distribution of water system radius in Tier 1 systems"}
d %>% 
...
```{r stratified random sampling, fig.cap = "Fig 11. Distribution of system radii with quartiles"}
d %>% 
...
```{r correlation of service connections and pop served, fig.cap="Fig 12. Service connections vs. population served"}
d %>% 
...
```{r correlation of service connections and pop served facet, fig.cap="Fig 13. Service connections vs. population served, faceted"}
d %>% 
  ggplot(aes(population_served_count, service_connections_count)) + 
  geom_point(aes(color = owner_type_code), alpha = 0.2) +
  rcartocolor::scale_color_carto_d() +
...
```{r interaction owner type code, fig.cap="Fig 13. Interaction plot: owner type code vs. service connections"}
d %>% 
...
```{r interaction wholesaler, fig.cap="Fig 14. Interaction plot: wholesaler vs. service connections"}
d %>% 
  ggplot(aes(service_connections_count, radius*km_to_mi)) + 
  geom_point(alpha = 0.2) + 
  geom_smooth(method = "lm", se = FALSE) +
...
```{r service conn v radius, fig.cap="Fig 14. Interaction plot: service area type code vs. service connections"}
...
```{r rf vip, fig.cap="Fig 15. Random forest variable importance"}
library(tidymodels)
library(vip)
...
```{r rf fit test residuals and error, fig.cap="Fig 16. Random forest test residuals, highlighted by ownership type"}
# predict on test set
rf_test_res <- test %>% 
  # select(radius) %>% 
...
```{r xgboost hyperparameter, fig.cap="Fig 17. XGBoost model performance across tuning grid"}
...
```{r xgboost vip and residuals and performance, fig.cap="Fig 18. XGBoost variable importance"}
...
```{r xgboost residuals and performance, fig.cap="Fig 19. XGBoost test residuals"}
# predict on test set
xgb_test_res <- test %>% 
  select(radius) %>% 
...
```{r lm fit, fig.cap="Fig 20. Linear model residuals"}
...
```{r lm spatial autocorrelation residuals, fig.cap="Fig 21. Spatial residuals (Tier 1 states)"}
...
```{r lm ci range distribution, fig.cap="Fig 22. Distribution of confidence interval range for modeled boundaries"}
t<- df %>% 
  select(pwsid, radius) %>% 
